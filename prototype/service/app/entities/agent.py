...
from app.utils.captioning import *
from app.utils.rag import *
from app.utils.risk import *
from app.utils.discrepancy import *
...
class Agent:
    def __init__(
        ...
        captioner : BlipForConditionalGeneration,
    ...
    def load_questions(self) -> None:
        ...
        for category in CATEGORIES:
            with open(f"app/data/questions/{category}.json", 'r', encoding='utf-8') as file:
    ...
    def produce_questionnaire(self, applicant : Applicant, category : str) -> list[dict]:
        '''
        Returns the traditional questions for a given category
        '''
        section = []
        for index, (factor, content) in enumerate(self.questions[category].items()):
            if index >= 10:
                break
            section.append({
                'questionId': index + 1,
    ...
    def save_answer(self, applicant : Applicant, factor : str, answer : int) -> None:
        '''
    ...
    def collect_geo_indicators(self, applicant : Applicant, municipality : int | str) -> None:
        '''
        Fetches the main risk indicators from the applicant's municipality
        '''
        if isinstance(municipality, int):
            municipalities = self.questions['personal']['personal_location']['options']
            if municipality >= len(municipalities):
                return
            municipality = municipalities[municipality] 
        demographics = {}
        for indicator in GEO_INDICATORS:
            content = pd.read_csv(f"app/data/demographics/{indicator}.csv", encoding='utf-8')
    ...
    def collect_social_media_posts(self, applicant : Applicant, posts : List[str]) -> None:
        ...
        captions = []
        for post in posts:
            image = Image.open(post).convert('RGB')
            caption = generate_caption(self.processor, self.captioner, image)
            captions.append(caption)
        applicant.posts = captions
...
You can only select from this list of risk factors (delimited by triple backticks):
```
{factor_list}```

You will be provided with user insights.
Identify all the risk factors present in a negative or risky way.

"""
        system += """Simply return in this JSON format:
[
    "factor_a",
    "factor_b", 
    "factor_c"
...
2. If none of the remaining factors meet the conditions:
   - reply with 'exit' only"""
    ...
    def select_question(self, applicant : Applicant) -> str:
        '''
...
{remaining_factors}"""
    ...
    def produce_next_question(self, applicant : Applicant, index : str = 0) -> dict:
        '''
        Produces the next question given the current questionnaire progress
        '''
        factor = self.select_question(applicant)
        content = {}
        if factor != 'exit':
            category = factor.split('_', 1)[0]
            question = self.questions[category][factor]['question']
    ...
    def build_record(self, applicant : Applicant) -> list[dict]:
        '''
    ...
    def compute_risk(self, applicant : Applicant) -> int:
        '''
        Computes a mortality risk score for an applicant based on the answers provided
        '''
        weights, combinations = load_weights()
        risk = mortality_risk(applicant.answers, weights, combinations)
        applicant.risk = risk
        return risk
    
    # -------------------------------------------------------------------------------------------------------------

    def identify_discrepancies(self, applicant : Applicant, min_confidence : float = 0.7) -> list[dict]:
        '''
        Identifies the answer-prediction discrepancies within a questionnaire 
        '''
        discrepancies = []
        for item in applicant.record:
            if 'prediction' in item and 'answer' in item:
                if item['confidence'] < min_confidence:
                    continue
                elif answer_discrepancy(self.embeddings, item['options'], item['answer'], item['prediction']):
                    discrepancies.append(item)
        applicant.discrepancies = discrepancies
